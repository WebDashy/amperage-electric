# Amperage Electric Co. — WebDashy Template

A multi-page, SEO-structured marketing site template for a licensed
electrical contractor, built for use as a live preview template in
[WebDashy](https://github.com/WebDashy/webdashy).

This is **Layout 7** of WebDashy's 8-layout template system — same page
structure convention as every other layout (home, services hub, one page
per service, about, projects, contact), a distinct bright/technical
visual language (Sora + Manrope + JetBrains Mono, electric-lime + navy,
hexagon icon badges, a "spec plate" nameplate-style stat card overlapping
the hero photo, a light stat-pill row instead of a dark stats bar) so it
reads as a genuinely different design choice next to the other layouts,
not a reskin.

Ships with real stock photography (sourced from Pexels, free for
commercial use, no attribution required) in `assets/img/` — hero and
project imagery are real photos, not CSS placeholder blocks.
...
assets/img/            stock photography — hero and project imagery
index.html             home
...
- The photography in `assets/img/` — swap for stock images matching the
  new category
...
Then open `http://localhost:4000/amperage-electric/`.
